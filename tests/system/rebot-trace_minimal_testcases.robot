...
run_process (verbose)                                                   run_process                                                     mode=verbose
run_process_redirect                                                    run_process_redirect
run_process_redirect (verbose)                                          run_process_redirect                                            mode=verbose
...
suite_teardown_failing_skipping                                         suite_teardown_failing_skipping
suite_teardown_failing_skipping (verbose)                               suite_teardown_failing_skipping                                 mode=verbose
...
Run Minimal Testcase
    ...
    ...    output matches.
    [Arguments]    ${testcase}    @{additional_args}    ${mode}=normal
    ...
    # robocop: off=VAR01  Intentionally empty variable.
    VAR    @{base_args}
    IF    "${mode}" == "verbose"  Append To List    ${base_args}    --verbose

    # First run robot-trace to generate both the expected output and the output.xml.
    VAR    ${output_xml}    ${TEMPDIR}${/}${testcase}${/}${mode}${/}output.xml
    ...
    ${normalized_expectation} =    Normalize Output    ${res.stdout}

    # Run rebot-trace and check the output matches the expectation.
    ${res} =  Run Process Check Output
    ...    rebot-trace
    ...    @{base_args}
    ...    ${output_xml}
